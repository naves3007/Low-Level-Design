...
void producer() {
  for (int i = 1; i <= 5; i++) {
    lock_guard<mutex> lock(m); // Lock_guard will automatically release the lock
...
void consumer(int id) {
  while (true) {
    unique_lock<mutex> lock(m); // Unique_lock allows us to manually control the
                                // locking and unlocking
    cv.wait(lock, [] {
      return !q.empty() || done;
    }); // Wait until there is something to consume or production is done
    if (!q.empty()) {
      int item = q.front();
      q.pop();
